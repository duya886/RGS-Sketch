...
uint32_t R[64]={2725628290, 1444492164, 1001369998, 2291192083, 3632694421, 3217476757, 962572696, 3523028760, 1564808096, 1686744101, 2572712615, 186525479, 1267589045, 2944996661, 3807606838, 2892595386, 1879920448, 3800713281, 4044617030, 1088214349, 2912533710, 4048616015, 223249368, 1094862426, 1603814236, 3377932768, 121561825, 2119485156, 1642325476, 1361943285, 3238649846, 992916862, 2726157025, 948801326, 1700806592, 2551402770, 1005395552, 682000371, 3023082996, 2349643312, 1249345355, 3442604002, 876940377, 3139269665, 1263700733, 3415088177, 3221633189, 2197631316, 3381857009, 3195477495, 895312444, 550032261, 2868005773, 3135351842, 3169035837, 349975359, 4280665935, 2689433230, 600974657, 1031368963, 896756897, 756356976, 1877735056, 2369702860};
...
private:
    ...
    inline static uint32_t getRegValue(uint32_t regStartByteIdx,uint32_t regStartBitIdx,uint8_t* regArray){
        return ((*(uint16_t*)(regArray+regStartByteIdx))>>regStartBitIdx)&0x1F;
    }
    inline static void setRegValue(uint32_t regStartByteIdx,uint32_t regStartBitIdx,uint32_t newValue,uint8_t* regArray){
    ...
    static double getTotalSpread(uint8_t* regArray){
        ...
        for(uint32_t groupIdx=0;groupIdx<REG_GROUP_NUM;groupIdx++){
            for(uint32_t regIdxInGroup=0;regIdxInGroup<REG_GROUP_SIZE;regIdxInGroup++){
                ...
                uint32_t regStartBitIdxInArray=groupIdx*5*REG_GROUP_SIZE+regIdxInGroup*5;
                uint32_t regValue= getRegValue(regStartBitIdxInArray/8,regStartBitIdxInArray%8,regArray);
    ...
    static double estimateBasedOnRegs(uint32_t key, uint8_t* regArray,double totalSpread){
        ...
        for(uint32_t virGroupIdx=0;virGroupIdx<V_REG_GROUP_NUM;virGroupIdx++){
            ...
            for(uint32_t regIdxInGroup=0;regIdxInGroup<REG_GROUP_SIZE;regIdxInGroup++){
                uint32_t regStartBitIdxInArray=phyGroupIdx*5*REG_GROUP_SIZE+regIdxInGroup*5;
                uint32_t regValue= getRegValue(regStartBitIdxInArray/8,regStartBitIdxInArray%8,regArray);
                sum+=1.0/pow(2,regValue);
...
public:
    ...
    void insert(const pair<uint32_t,uint32_t>& pkt){
        ...
        uint32_t groupStartBitIdxInArray=phyGroupIdx*5*REG_GROUP_SIZE;
        uint32_t regStartBitIdxInArray=groupStartBitIdxInArray+regIdxInGroup*5;
        uint32_t regStartByteIdx=regStartBitIdxInArray/8;
        uint32_t regStartBitIdx=regStartBitIdxInArray%8;
        uint32_t oriRhoValue= getRegValue(regStartByteIdx,regStartBitIdx,regArray);

        if(rhoValue>oriRhoValue){
            uint32_t wordLeftBits=64-groupStartBitIdxInArray%8;//the unused bits in the word
            uint64_t* ptr=(uint64_t*)(regArray+groupStartBitIdxInArray/8);

            uint64_t value=*ptr;
            value=value>>(groupStartBitIdxInArray%8);
            uint32_t readREGCount=0;
            uint32_t sum=0.0;
            while(readREGCount<REG_GROUP_SIZE){
                while(wordLeftBits>=5 && readREGCount<REG_GROUP_SIZE){
                ...
                if(readREGCount<REG_GROUP_SIZE){
                    ptr++;
                    uint32_t lastValue=value;
                    value=*ptr;
                    if(wordLeftBits>0){
                        uint32_t bitsOfRegInNewWord=5-wordLeftBits;
                        uint32_t regVal=(((uint32_t)value&((1<<bitsOfRegInNewWord)-1))<<wordLeftBits)|lastValue;

                        if(regVal!=maxRho){//when the reg value is maxRho, it cannot be updated again
                            sum+=1<<(maxRho-1-regVal);
                        }

                        value=value>>bitsOfRegInNewWord;
                        wordLeftBits=64-bitsOfRegInNewWord;
                        readREGCount++;
                    }else{
                        wordLeftBits=64;
                    }
            ...
            double updateValue= (double)(((uint64_t)REG_GROUP_SIZE)<<(maxRho-1))/(double)sum;

            uint32_t bktIdx=hashValue1%BUCKET_NUM;
            if(updateBucket(pkt,(hashValue1+99985157*hashValue2),bktIdx,updateValue)){
                setRegValue(regStartByteIdx,regStartBitIdx,rhoValue,regArray);
            }
        }
        ...
        for(uint32_t sketchIdx=0;sketchIdx<sketches.size();sketchIdx++){
            ...
            for(uint32_t groupIdx=0;groupIdx<REG_GROUP_NUM;groupIdx++){
                for(uint32_t regIdxInGroup=0;regIdxInGroup<REG_GROUP_SIZE;regIdxInGroup++){
                    ...
                    uint32_t curRegValue= getRegValue(regStartByteIdx,regStartBitIdx,newRegArray);
                    uint32_t sketchRegValue= getRegValue(regStartByteIdx,regStartBitIdx,sketches[sketchIdx].regArray);
                    if(sketchRegValue>curRegValue){
                        setRegValue(regStartByteIdx,regStartBitIdx,sketchRegValue,newRegArray);
                    }
                }
            }
        }
...
//     const uint32_t regGroupNum_500KB=66560;
    ...
    const uint32_t regGroupSize=4;
    const uint32_t vRegGroupNum=64;

    //100kB
    const uint32_t bucketNum_100KB=560;
    const uint32_t regGroupNum_100KB=13312*2;
    //200kB
    const uint32_t bucketNum_200KB=1120;
    const uint32_t regGroupNum_200KB=26624*2;
    //300kB
    const uint32_t bucketNum_300KB=1680;
    const uint32_t regGroupNum_300KB=39936*2;
    //400kB
    const uint32_t bucketNum_400KB=2240;
    const uint32_t regGroupNum_400KB=53248*2;
    //500kB
    const uint32_t bucketNum_500KB=2800;
    const uint32_t regGroupNum_500KB=66560*2;
